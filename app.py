...
CATEGORY_SHEET_COLS = {
    ...
    "keyword": "Title Keyword",       # fallback only
...
def match_category_id(title, category_df, keyword_col, id_col,
    ...
    """
    Resolves Category ID. Two strategies, tried in order:
      1. COMPOSITE KEY exact match (PRIMARY): AgeGroup-Gender-ArticleGroup-
         ArticleType-ActivityGroup-ProductDivision, normalized (case-
         insensitive, whitespace-collapsed) before comparing.
      2. TITLE keyword match (fallback), used only if the composite key
         doesn't resolve anything.
    Returns "" (leaving the cell blank) if neither strategy matches --
...
def match_size_chart_template(size_chart_key, size_chart_template_df, key_col, attr_col):
    ...
    Template value formatted as "sizechart=<value>". Returns "" (leaving
    the cell blank) if nothing matches -- never overwrites with a guess.
    """
...
def build_upload_sheet(master_df, image_df, size_chart_template_df, category_df,
    ...
    for group_key_val, group_df in master_df.groupby("_group_key", sort=False):
        ...
        # ActivityGroup-ProductDivision composite key (primary), Title keyword
        # match (fallback). Never overwrites with a bad guess -- blank if
        # unmatched. ---
        category_key = build_category_key(
...
if category_file is not None:
    ...
    st.caption(
        ...
        "AgeGroup-Gender-ArticleGroup-ArticleType-ActivityGroup-ProductDivision "
        "from your Master Sheet (e.g. \"Adult-Men-Tops-Tee-Auto-Apparel\"). "
        "Title Keyword is a fallback match, used only if the composite key "
        "doesn't resolve anything."
    )
    ...
    _cat_key_choice = st.selectbox(
        "Category Key column (AgeGroup-Gender-ArticleGroup-ArticleType-ActivityGroup-ProductDivision)",
    ...
    with cc1:
        ...
        category_keyword_col = st.selectbox(
            "Title keyword column (fallback match)",
